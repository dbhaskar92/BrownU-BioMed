...
random_polarities = [0.25*1e-2 0.3*1e-2 0.35*1e-2 0.4*1e-2 0.45*1e-2 0.5*1e-2 0.55*1e-2 0.6*1e-2];
random_densities = [25, 50, 100, 200, 250];

conditions_den = ["1", "2", "3", "4", "5"];
conditions_pol = ["1", "2", "3", "4", "5", "6", "7", "8"];

for den_vals = 1 : length(random_densities)
    
    for pol_vals = 1 : length(random_polarities)

        cond_str = strcat(conditions_den(den_vals), "_den_", conditions_pol(pol_vals), "_pol");
        mkdir(char(cond_str));
